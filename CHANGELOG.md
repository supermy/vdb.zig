...
### HTTP 服务器 libevent 重构

- **libevent evhttp 替代 POSIX socket**：使用 libevent 的 `evhttp` API（`event_base_new` + `evhttp_new` + `evhttp_bind_socket` + `event_base_dispatch`）替代手动 `socket/bind/listen/accept` + 每连接线程模型。
- **事件驱动架构**：单线程事件循环处理所有连接，消除线程 spawn/destroy 开销，避免慢请求（benchmark）阻塞其他连接。
- **C 绑定封装**：定义 `event_base`、`evhttp`、`evhttp_request`、`evbuffer`、`evkeyvalq` opaque 类型和 `le` 命名空间，封装所有 libevent API 调用。
- **请求体读取**：通过 `evbuffer_pullup` + `evbuffer_get_length` 从 evhttp 输入缓冲区读取 POST body，分配器复制后传递给 handler。
- **响应发送**：`evSendReply` / `evSendJson` / `evSendJsonError` / `evSendCors` 辅助函数，通过 `evhttp_add_header` 设置响应头 + `evbuffer_add` 填充 body + `evhttp_send_reply` 发送。
- **build.zig 链接 libevent**：`server_mod.linkSystemLibrary("event", .{})` 自动添加 `-I` / `-L` / `-levent` 编译参数（通过 pkg-config）。

### TDD 测试审查

- **8 层测试体系全部通过**：unit（50+ 测试）+ integration + smoke + regression + acceptance + system + e2e + server。
- 测试覆盖：schema 验证、SIMD 运算、索引 insert/search、RaBitQ 召回、SQ8 精排、分层 K-Means、线程池、GPU fallback、存储 roundtrip、SQL 谓词、batchSearch、FastScan 路径一致性等。
